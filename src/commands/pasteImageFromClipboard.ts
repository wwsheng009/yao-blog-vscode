...
import * as clip from "clipboard-data";

// let clip: any;

export function pasteImageFromClipboardActivate(
  context: vscode.ExtensionContext
) {
  if (!clip) {
    // clip = require("clipboard-data");
  }
  let pasteImageFromClipboardDisposable = vscode.commands.registerCommand(
    "writeCnblog.pasteImageFromClipboard",
    () => {
      try {
        let textEditor = vscode.window.activeTextEditor;
        if (textEditor) {
          let pngData = clip.getImage();
          let imageShortPath = path.join(
            imageDirName,
