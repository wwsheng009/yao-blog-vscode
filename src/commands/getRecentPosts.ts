...
export function getRecentPostsActivate(context: vscode.ExtensionContext) {
  let getRecentPostsDisposable = vscode.commands.registerCommand(
    "writeCnblog.getRecentPosts",
    () => {
      vscode.window
        .withProgress(
          {
          ...
          async (progress, token) => {
            ...
            try {
              let recentPostCount = BlogConfig.instance.recentPostCount();
              if (!recentPostCount) {
                recentPostCount = 100;
              }
              ...
              posts.forEach(
                (p) => (p.description = decodeURIComponent(p.description))
              );
