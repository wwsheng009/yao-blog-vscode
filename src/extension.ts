...
export async function activate(context: vscode.ExtensionContext) {
  BlogConfig.init(context);
  const settings = BlogConfig.instance;

  // Register commands to save and retrieve token
  vscode.commands.registerCommand("writeCnblog.setPassword", async () => {
    const tokenInput = await vscode.window.showInputBox();
    await settings.setPassword(tokenInput);
  });
  vscode.commands.registerCommand("writeCnblog.getPassword", async () => {
    const tokenOutput = await settings.password();
    console.log(tokenOutput);
  });

  blogPostProvider.initialize(context);
  vscode.window.createTreeView("blogPostExplorer", {
    treeDataProvider: blogPostProvider,
  });
  vscode.window.createTreeView("blogCategoriesExplorer", {
    treeDataProvider: blogCategoriesProvider,
  });
  ...
  try {
    ...
    const extensionId = "yao-blog-vscode";
