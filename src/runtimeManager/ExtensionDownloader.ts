...
export class ExtensionDownloader {
  ...
  public installRuntimeDependencies(): Promise<boolean> {
    this.logger.log("Installing write-yao-blog dependencies...");
    this.logger.show();
    ...
    return util
      .touchInstallFile(util.InstallFileType.Begin)
      .then(() => {
      ...
      .then(() => {
        ...
        if (platformInfo.distribution) {
          telemetryProps["platform.distribution"] =
            platformInfo.distribution.toTelemetryString();
        }
      ...
      .then(() => {
        ...
        return util
          .deleteInstallFile(util.InstallFileType.Begin)
          .catch((error) => {});
      })
      .then(() => {
        return success;
