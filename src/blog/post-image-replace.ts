...
export class PostImageReplace {
  ...
  public async toRemote(post: string): Promise<string> {
    ...
    for (let index = 0; index < images.length; index++) {
      ...
      if (urls) {
        ...
        let remotePath = imageIndex
          ? imageIndex.remote
          : await this.imageUpload(imageUrl);
        if (remotePath) {
  ...
  public async toLocal(post: string): Promise<string> {
    ...
    for (let index = 0; index < images.length; index++) {
      ...
      if (urls) {
        ...
        let localPath = imageIndex
          ? imageIndex.local
          : await this.imageDown(imageUrl);
  ...
  private imageDown(url: string): Promise<string> {
    const rpcUrl = BlogConfig.instance.rpcUrl() + "";
    const host = getBaseUrl(rpcUrl);
