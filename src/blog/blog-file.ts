...
import { blogDirName, blogIndexName, fileExt } from "../constants";
import {
  PostIndexInfo,
  PostFile,
  PostBaseInfo,
...
export class BlogFile {
  ...
  private get indexId(): number {
    return this.postIndexs.length === 0
      ? 1
      : this.postIndexs[this.postIndexs.length - 1].id + 1;
  }
   ...
   * @param titleIn
   */
  private hasIndexByTitle(titleIn: string): boolean {
    let title = titleIn;
    if (title.indexOf(".") !== -1) {
      let info = extractFilenameParts(title);
      title = info.basePart;
    }
  ...
  private updateOrAddPostIndex(postIndex: PostIndexInfo): PostIndexInfo {
    let oldPostIndex = this.postIndexs.find(
      (p) => p.postid === postIndex.postid
    );
    if (oldPostIndex && postIndex.postid) {
      postIndex.id = oldPostIndex.id;
      if (oldPostIndex.title === oldPostIndex.remoteTitle) {
        //不相等不要覆盖
      ...
      oldPostIndex.ext = postIndex.ext;
    } else {
  ...
  private readPostFiles(): Array<PostFile> {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (stat.isFile()) {
        ...
        let ext = child.substring(lastIndex);
        postFiles.push({
  ...
  private createBlogPostFileByPostId(postId: string): BlogPostFile {
    ...
    if (!postIndex || !postId) {
      postIndex = { id: this.indexId, postid: postId, title: "", ext: "" };
    }
  ...
  public async pullPost(post: PostStruct): Promise<void> {
    ...
    // post.description = decodeURIComponent(post.description);
    let description = await postImageReplace.toLocal(post.description);

    blogPostFile.updatePost(
      post.title,
      description,
      post.type || fileExt.substring(1)
    );
  ...
  public async getPost(id: number): Promise<PostStruct> {
    ...
    let description = await postImageReplace.toRemote(
      blogPostFile.description()
    );
    let categories: (string | TreeItemLabel)[] = postIndex.categories
      ? postIndex.categories
      : [];
    ...
    let post: PostStruct = {
      postid: postIndex.postid,
      title: postIndex.title,
      type: postIndex.ext ? postIndex.ext.substring(1) : "",
