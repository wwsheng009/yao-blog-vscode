...
export class BlogPostProvider implements vscode.TreeDataProvider<BlogPostItem> {
  ...
  getChildren(
    element?: BlogPostItem | undefined
  ): vscode.ProviderResult<BlogPostItem[]> {
    ...
    if (element && element.postBaseInfo && element.postBaseInfo.categories) {
      ...
      .map<BlogPostItem>((postBaseInfo) => {
        return {
          ...
          command: {
            ...
            title: "打开文章",
            arguments: [vscode.Uri.file(postBaseInfo.fsPath)],
          },
